...
def get_occ_info(index):
    ...
    for cam_name in camera_names:
        ...
        # print("ego2cam shape", ego2cam.shape)
...
def get_san_seg_result(proj_img_index, proj_uv, filenames, args):
    cat_seg_dir = args.seg_root
    seg_results = []
    for filename in filenames:
        seg_path = os.path.join(cat_seg_dir, filename.replace('.jpg', '.png'))
        ...
        for valid_indice in valid_indices:
            if proj_info[valid_indice, 0] < 0:
                ...
                if new < old:
                    proj_info[valid_indice, 0] = i
                    proj_info[valid_indice, 1:] = uv_coords[valid_indice, :]
        # proj_info[valid_indices, 0] = i
        # proj_info[valid_indices, 1:] = uv_coords[valid_indices, :]
    
    proj_mask = (proj_info[:, 0] < 0) | (proj_info[:, 1] < 0) | (proj_info[:, 2] < 0)
    proj_mask = ~proj_mask
    # print("proj_info shape", proj_info.shape)
    # print("proj_info shape", proj_info[mask].shape)
    # print(proj_info[~mask][:5, :])
    # exit()
...
def main(nusc, val_list, indice, nuscenesyaml, args, config):
    ...
    while True:
        if lidar_data['is_key_frame']:
            ...
            # occ_mask = np.zeros((voxel_coords.shape[0], )).astype(bool)
            index = lidar_data['sample_token']
            inputs, filenames, scene_name = get_occ_info(index)
            # occlusion_mask = single_frame(inputs)
            # occlusion_mask = occlusion_mask.astype(bool)

            # voxel_coords_in_range = voxel_coords[range_mask]
            # occ_mask[range_mask] = occlusion_mask[voxel_coords_in_range[:, 0], voxel_coords_in_range[:, 1], voxel_coords_in_range[:, 2]]
    ...
    while int(i) < 10000:  # Assuming the sequence does not have more than 10000 frames
        ...
        pcd_np = scene_semantic_points[:, :3].copy()
        ...
        # nearest points to voxel center
        fov_voxels = (occ_index + 0.5) * voxel_size + np.array([pc_range[0], pc_range[1], pc_range[2]])
        dense_voxels = fov_voxels
        sparse_voxels_semantic = scene_semantic_points.copy()
        ...
        dense_semantic = lidar_info[np.array(indices)].astype(np.float16)

        occ_mask[tuple(occ_index.T)] = dense_semantic
...
if __name__ == '__main__':
    ...
    parse.add_argument('--save_path', type=str, default='/mnt_gx/yz/dataset/occ3d/san_seg_gts_qwen_nearest')
    parse.add_argument('--seg_root', type=str, default='/mnt_gx/yz/dataset/occ3d/san_seg_qwen_scene')
    parse.add_argument('--start', type=int, default=0)
    parse.add_argument('--end', type=int, default=3)
    parse.add_argument('--dataroot', type=str, default='/mnt_gx/yz/dataset/occ3d')
