...
def main(nusc, val_list, indice, nuscenesyaml, args, config):
    ...
    while True:
        if lidar_data['is_key_frame']:
            ...
            proj_img_index = proj_matrix[:, 0].astype(np.int32)
    ...
    while int(i) < 10000:  # Assuming the sequence does not have more than 10000 frames
        ...
        ############################# generate img_dict for gt ##############################
        img_names_all = np.unique(lidar_info[:, -1])
        # img_index_map to save, img_index_map_inv to process info
        img_index_map = {}
        img_index_map_inv = {}
        num = 0
        while num < img_names_all.shape[0]:
            img_index_map[num] = img_names_all[num]
            img_index_map_inv[img_names_all[num]] = num
            num += 1
        lidar_info[:, -1] = np.array([img_index_map_inv[img_name] for img_name in lidar_info[:, -1]])
        ...
        occ_mask = np.zeros(occ_size).astype(bool)
        voxel_infos = np.zeros((occ_index.shape[0], 4)).astype(np.float16)
        
        # nearest points to voxel center
        occ_mask[occ_index[:, 0], occ_index[:, 1], occ_index[:, 2]] = 1
        ...
        voxel_infos = lidar_info[np.array(indices)].astype(np.float16)

        ###################### save img_index_map, occ_mask, voxel_infos to pickle ######################
        save = {}
        save['img_dict'] = img_index_map
        save['occ_mask'] = occ_mask
        save['voxel_infos'] = voxel_infos
...
if __name__ == '__main__':
    ...
    parse.add_argument('--save_path', type=str, default='data/occ3d/san_gts_feat')
