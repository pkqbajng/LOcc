...
class OVOHead(BaseModule):
    ...
    def loss_language_field(self, language_field, ovo_gt, mask, img_metas=None):
        if self.scene_specific:
            # To do: bs > 1
            text_embedding_file = img_metas.data[0][0]['text_embedding_file']
        ...
        if self.balance_cls_weight:
            ...
            for unique_index in unique_indexes:
                loss_index = torch.sum(1 - loss_language_field[indexes==unique_index]) / np.sum(indexes==unique_index)
                loss += loss_index
            loss = loss / len(self.text_embedding)
        else:
