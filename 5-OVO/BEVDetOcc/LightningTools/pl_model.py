...
class pl_model(LightningBaseModel):
    def __init__(
        ...
        self.train_metrics = Metric_SSC(num_classes=config['num_classes'], use_image_mask=config['use_mask'])
        self.val_metrics = Metric_SSC(config['num_classes'], use_image_mask=config['use_mask'])
        self.test_metrics = Metric_SSC(config['num_classes'], use_image_mask=config['use_mask'])
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.config['visualize'] is not None:
            print(output_dict['img_metas'])
            visualize_root = self.config['visualize']
            occ_gt_path = output_dict['img_metas'][0][0]['occ_gt_path']
            scene_name, token = occ_gt_path.split('/')[-2], occ_gt_path.split('/')[-1]
