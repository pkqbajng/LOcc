...
class SAN(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        temp_mask = mask_preds[-1].permute(0, 2, 3, 1).sigmoid()
        medium_feat = torch.matmul(temp_mask.unsqueeze(3), mask_embs[-1].unsqueeze(0).unsqueeze(0))
        medium_feat = medium_feat.squeeze(3)
        if self.training:
            ...
            processed_results[-1]["medium_feat"] = medium_feat
