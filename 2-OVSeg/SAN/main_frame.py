...
def parse_config():
    ...
    parser.add_argument("--output_root", default="data/occ3d/san_qwen_frame")
    ...
    for i in tqdm(range(args.start, args.end)):
        ...
        if os.path.exists(scene_dir):
            ...
            for index in tqdm(index_list):
                ...
                frame_vocab = load_txt(os.path.join(args.vocab_root, scene_name, index + '.txt'))
                frame_vocab.append('sky')
                mapping = {i: scene_vocab.index(category) for i, category in enumerate(frame_vocab)}
                vocabulary = frame_vocab
                ...
                for cam_name in camera_names:
                    ...
                    seg_result = result['sem_seg']

                    updated_seg_result = np.copy(seg_result)
                    for old_index, new_index in mapping.items():
                        updated_seg_result[seg_result == old_index] = new_index
                    final_result = updated_seg_result
