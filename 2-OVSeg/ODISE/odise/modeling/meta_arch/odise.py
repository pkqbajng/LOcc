...
class CategoryODISE(ODISE):
    ...
    def forward_img_feature(self, batched_inputs):
        ...
        for mask_pred_result, input_per_image, image_size in zip(
                ...
                if self.sem_seg_postprocess_before_inference:
                    ...
                    img_feat = torch.einsum("qc,qhw->chw", mask_embed.squeeze(0), mask_pred_result)
                    img_feats.append(img_feat)
        
        return img_feats[0]
    
    def setup_vocabulary(self, vocabulary):
        self.category_head.test_labels = vocabulary
        self.clip_head.test_labels = vocabulary
    ...
    def forward(self, batched_inputs, vocabulary=None):
        ...
        if vocabulary is not None:
            self.setup_vocabulary(vocabulary=vocabulary)
...
class CategoryEmbed(nn.Module):
    ...
    def forward_text(self, test_labels):
        labels = test_labels
